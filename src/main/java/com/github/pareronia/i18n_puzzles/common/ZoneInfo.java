/* https://bmsi.com/java/ZoneInfo.java
 * adapted to work with Version 2 format
 */
...
class TZType implements Serializable {
  ...
  TZType(final String name,final int offset,final boolean dst) {
    this.name = name;
    this.offset = offset;
    this.isdst = dst;
  }
...
public class ZoneInfo extends TimeZone {
  ...
  public ZoneInfo(final File f) throws IOException {
    ...
    try (ds) {
      ...
      transTimes = new long[timecnt];
      for (int i = 0; i < timecnt; ++i) {
        transTimes[i] = ds.readLong();
    }
      transTypes = new byte[timecnt];
      ds.readFully(transTypes);

      // load TZ type data
      final int[] offset = new int[typecnt];
      final byte[] dst = new byte[typecnt];
      final byte[] idx = new byte[typecnt];
      for (int i = 0; i < typecnt; ++i) {
    offset[i] = ds.readInt();
    dst[i] = ds.readByte();
    idx[i] = ds.readByte();
      }
      final byte[] str = new byte[charcnt];
      ds.readFully(str);
    ...
    tz[i] = new TZType(new String(str,pos,end-pos),offset[i],dst[i] != 0);
      ...
      leapSecs = new long[leapcnt * 2];
      for (int i = 0; leapcnt > 0; --leapcnt) {
    leapSecs[i++] = ds.readLong();
    leapSecs[i++] = ds.readLong();
      }
    }
    ...
    for (int i = 0; i < 9; i++) {
      final TZType currTz = getTZ(ts + secsPerThreeMonths*i);
      if (!currTz.isdst) {
    normaltz = currTz;
    break;
      }
   ...
   */
  public ZoneInfo(final String tzname) throws IOException {
   ...
   */
  @Override public String getDisplayName(final boolean daylight, final int style, final Locale locale) {
    //System.err.println("ZoneInfo.getDisplayName");
    if (style == SHORT) { // only SHORT names available in zoneinfo
      if (!daylight) {
        return normaltz.name;
    }
      for (final TZType t: tz) {
        if (t.isdst) {
            return t.name;
        }
    }
  ...
  @Override public void setRawOffset(final int millis) {
  ...
  /** Return the offset from UT for a calendar date and time.
      ...
      getRawOffset().
   */
  @Override public int getOffset(final int era,final int year,final int month,final int day,final int dow,final int millis) {
    if (era != GregorianCalendar.AD) {
        return getRawOffset();
    }
    final int secs = millis/1000;
    final tm then = new tm(year - 1900,month,day,secs);
    long ts;
    try {
      ts = mktime(then,true);
    }
    ...
    for (int y = leapSecs.length; (y-=2) >= 0; ) {
      final long ls_trans = leapSecs[y];
      final long ls_corr = leapSecs[y+1];
      if (ts >= ls_trans) {
    offset -= ls_corr;
    break;
      }
  ...
  @Override public boolean inDaylightTime(final Date d) {
    final TZType tz = getTZ((int)(d.getTime()/1000));
  ...
  */
  ...
  public static class tm implements Comparable<tm> {
     ...
     */
    public tm(final int year,final int mon,final int day,final int tm_secs) {
      tm_year = year;
      tm_mon = mon;
      tm_mday = day;
      setSecs(tm_secs);
    ...
    public void setSecs(final int tm_secs) {
      //this.tm_secs = tm_secs;
      tm_hour = tm_secs / SECSPERHOUR;
      final int rem = tm_secs % SECSPERHOUR;
      tm_min =  rem / SECSPERMIN;
      tm_sec = rem % SECSPERMIN;
    }

    @Override
    public int compareTo(final tm t) {
     ...
     */
    public void setClock(final long clock,final int offset) {
   ...
   */
  public tm gmtime(final long clock) {
    final tm t = new tm();
    t.setClock(clock,0);
    t.tm_zone = "GMT";
    return t;
  ...
  /** Compute UTC from clock.  This includes leap second corrections if
      ...
      @return a new tm object with all time fields computed.
   */
  public tm utctime(final long clock) {
    final tm t = new tm();
   ...
   */
  public int localtime(final long clock,final tm t) {
    return timesub(clock,getTZ(clock),t);
  }
    
  /** Lookup which timezone a given instant should use.  */
  private TZType getTZ(final long clock) {
  ...
  /** Compute local time from seconds since the epoch.
      @param clock  seconds since 1970
   */
  public tm localtime(final long clock) {
    final tm t = new tm();
    localtime(clock,t);
    return t;
  }
   ...
   */
  public long mktime(final tm yourtm) {
    return mktime(yourtm,false);
  }

  private long mktime(final tm yourtm,final boolean raw) {
    int t = 0;
    int bits = 31;
    final int offset = getRawOffset() / 1000;
    final tm mytm = new tm();
    // use binary search
    ...
    for (;;) {
      ...
      final int direction = mytm.compareTo(yourtm);
      if (direction == 0) {
   ...
   */
  private int timesub(final long clock, final TZType tz, final tm t) {
    boolean     hit = false;
  ...
  public static void main(String[] argv) throws Exception {
    ...
    for (final String s: argv) {
      final ZoneInfo tz = new ZoneInfo(s);
  ...
  static class TZDump {
    ...
    private static void dumpzone(final String tzname) throws IOException {
      final TZDump tz = new TZDump(tzname);
